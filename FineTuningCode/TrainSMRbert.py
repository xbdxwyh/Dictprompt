...
def tokenize_function(examples,max_length=256):
    tokenized_sentence = tokenizer([examples['FalseSent']] * 3,[examples['OptionA'],examples['OptionB'],examples['OptionC']],padding='max_length',max_length = max_length,truncation = True)
...
if __name__ == '__main__':
    ...
    model = BertForMultipleChoice.from_pretrained(args.model).cuda()
    ...
    with open('ALL_RESULTS\\SMR_'+args.model+'_7_26.txt','a+') as f:
        f.writelines([str(i)+'\n' for i in [max_acc]])
    # save best model
    with open('ALL_RESULTS\\SMR_'+args.model+'_7_26.txt', 'r') as f:
        data = f.readlines()
