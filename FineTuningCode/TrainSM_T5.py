...
from ..models.T5ForSequenceClassification import T5ForSequenceClassification


def compute_metrics(eval_pred):
    predictions, labels = eval_pred
    prediction_labels = predictions[0].argmax(-1)
...
def tokenize_function(examples,max_length=256):
    tokenized_sentence = tokenizer("<extra_id_0> "+examples['sent0'],examples['sent1'],padding='max_length',max_length = max_length,truncation = True)
...
if __name__ == '__main__':
    ...
    model = T5ForSequenceClassification.from_pretrained(args.model).cuda()
    ...
    with open('ALL_RESULTS\\SM_'+args.model+'_7_30.txt','a+') as f:
        f.writelines([str(i)+'\n' for i in [max_acc]])
    # save best model
    with open('ALL_RESULTS\\SM_'+args.model+'_7_30.txt', 'r') as f:
        data = f.readlines()
