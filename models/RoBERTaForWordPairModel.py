...
class RobertaForWordPairClassification(RobertaPreTrainedModel):
...
class RobertaForWordPairEmbeddings(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.roberta = RobertaModel(config, add_pooling_layer=False)
...
class RobertaForWordPairEmbeddingsv2(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.roberta = RobertaModel(config, add_pooling_layer=False)
    ...
    def forward(
        ...
        # Calculate outputs using activation
        layer1_results = self.activation(self.linear_diff(diff))
        logits = self.linear_seperator(layer1_results)

        return WordEmbdPairOutput(
            embd_word1 = word1s,
            embd_word2 = word2s,
            hidden_states = self.linear_diff(diff),
            logits = logits
        )